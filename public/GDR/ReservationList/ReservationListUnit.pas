...
procedure FreeReservationListForm;
function GetReservationListForm : TReservationListForm;
...
var
  GReservationListForm: TReservationListForm;

{$R *.dfm}

function GetReservationListForm : TReservationListForm;
begin
  if not Assigned( GReservationListForm ) then
    GReservationListForm := TReservationListForm.Create( nil );
  Result := GReservationListForm;
end;

procedure FreeReservationListForm;
begin
  if Assigned( GReservationListForm ) then
  begin
    GReservationListForm.ShowPanel( nil );
    FreeAndNil( GReservationListForm );
  end;
end;
...
procedure TReservationListForm.AfterEventNotify(AEventID: Cardinal; AData: TObserverData);
var
  sd : TSelectData;
begin
  case AEventID of
    OB_Event_DataRefresh_Month2_List,
    OB_Event_DataRefresh_Month2_ProcessList :
      begin
        sd := TSelectData( AData );
        if Assigned( sd ) then
        begin  // data 출력
          FWorkDay := sd.Date;
          //DecodeDate(FWorkDay, FYear, FMonth, FDay);
          RefreshData;
        end;
      end;
...
procedure TReservationListForm.all_checkClick(Sender: TObject);
var
  sd : TSelectData;
begin
  sd := TSelectData.Create( FWorkDay );
  try
    FObserver.BeforeAction(OB_Event_DataRefresh_Month2_ProcessList, sd);
    try
      ...
      FObserver.AfterAction(OB_Event_DataRefresh_Month2_ProcessList, sd);
    end;
  finally
    FreeAndNil( sd );
  end;
...
procedure TReservationListForm.BeforeEventNotify(AEventID: Cardinal;
  AData: TObserverData);
begin

end;

procedure TReservationListForm.ButtonClickEvent(ASender: TObject);
var
  t, r : Integer;
...
begin // 내원요청/내원확인/진료완료  button click시 발생하는 event
  ...
  begin
    ...
    try
      case t of
        Index_tag_내원확인 : status := rrs내원확정;// status := rrs진료대기;
      else // Index_tag_진료완료
        status := rrs진료완료;
      end;
...
procedure TReservationListForm.ButtonClickEventThread(ASender: TObject);
begin
...
function TReservationListForm.CheckFilterData: Boolean;
var
  status : TRnRDataStatus;
...
procedure TReservationListForm.ClearGrid;
var
  i, j : Integer;
  o : TObject;
begin
...
constructor TReservationListForm.Create(AOwner: TComponent);
begin
  inherited;
  Foldrow4Hint := -1;
  FRoomFilter := '';
  ...
  GetRREnv.GridInfoList.GetGridInfo( Grid_Information_ReservationList_ID ).SetGridInfo( FSortType, listgrid);

  SetGridUI;
end;

destructor TReservationListForm.Destroy;
begin
...
procedure TReservationListForm.ec_btnClick(Sender: TObject);
begin
  FObserver.BeforeAction(OB_Event_ExpandCollapse_RR_RequestReservationList);
  try
    { TODO : 접었다/폈다 button의 상태를 변환 시켜야 한다 }
  finally
    FObserver.AfterAction(OB_Event_ExpandCollapse_RR_RequestReservationList);
  end;
end;

procedure TReservationListForm.filter_btnClick(Sender: TObject);
var
  roominfo : TRoomInfo;
  rect : TRect;
  sd : TSelectData;
...
function TReservationListForm.FindID4Grid(ARnRData: TRnRData;
...
function TReservationListForm.FindObject4Grid(AObj: TObject; var ARow,
...
procedure TReservationListForm.listgridDblClick(Sender: TObject);
var
  rowdata : TDispData;
...
procedure TReservationListForm.listgridDrawCell(Sender: TObject; ACol,
...
procedure TReservationListForm.listgridFixedCellClick(Sender: TObject;
  ACol, ARow: Integer);
var
  sd : TSelectData;
begin
  ...
  try
    ...
    try
      ...
      else if ACol = FDrawGrid.Col_Index_Time then // 예약 일시 sort
      begin
      ...
      FObserver.AfterActionASync( OB_Event_DataRefresh_Month2_ProcessList, sd );
    end;
  finally
    FreeAndNil( sd );
  end;
...
procedure TReservationListForm.listgridMouseActivate(Sender: TObject;
...
procedure TReservationListForm.listgridMouseLeave(Sender: TObject);
begin
  BalloonHint1.HideHint;
  Foldrow4Hint := -1;
end;

procedure TReservationListForm.listgridMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
var
  c, r : Integer;
  h : string;
...
procedure TReservationListForm.listgridMouseUp(Sender: TObject;
...
begin
  ...
  begin
    ...
    begin
      ...
      listgrid.Invalidate;
      GetRREnv.GridInfoList.GetGridInfo( Grid_Information_ReservationList_ID ).AssignGridInfo( FSortType, listgrid);
    end;
  end;
end;

procedure TReservationListForm.listgridTopLeftChanged(Sender: TObject);
begin
  BalloonHint1.HideHint;
  Foldrow4Hint := -1;
...
procedure TReservationListForm.RefreshData;
var
  step : Integer;
  rowindex : Integer;
  d : TDateTime;
  ...
  try
    ...
    begin
      ...
      case FSortType of
        ...
        Sort_Time_Ascending   : RR_DB.IndexName := 'visitIndex'; // 예약일시 순
        ...
        RR_DB.IndexName := 'visitIndexdesc'; // 방문 시간/예약일시 역순
      end;
...
procedure TReservationListForm.SetGridUI;
var
  i : Integer;
  str : string;
...
procedure TReservationListForm.ShowPanel(AParentControl: TWinControl);
begin
...
procedure TReservationListForm.StatusChangeNotify(AData: TRnRData;
...
procedure TReservationListForm.VisibleGridButton(AVisible: Boolean);
// grid에 출력되어 있는 모든 button들의 visible값을 지정된 값으로 설정 한다.
var
  i : Integer;
  dd : TDispData;
begin
  ...
  try
    ...
    begin
      ...
      if Assigned( dd ) then
      begin
        dd.Button_진료완료.Visible := AVisible;
        dd.Button_내원확인.Visible := AVisible;
      end;
    end;
  finally
    listgrid.Perform(WM_SETREDRAW, 1, 0);   // endupdate
