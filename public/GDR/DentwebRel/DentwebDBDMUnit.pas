unit DentwebDBDMUnit;
(* Dentweb DB에 직접 연결하여 쿼리 등을 통해서 환자 조회 등의 기능을 수행함 *)
...
uses
  ...
  GDThread, RestCallUnit, BridgeCommUnit, DentwebConnStrManager;

type
  TDentwebDBDM = class(TDataModule)
  private
...
constructor TDentwebDBDM.Create(AOwner : TComponent);
begin
  inherited;
  FActive := False;
...
destructor TDentwebDBDM.Destroy;
begin
  Stop;
...
function TDentwebDBDM.FindPatient(AEvent0: TBridgeResponse_0) : TBridgeRequest_1;
  ...
  begin
    ...
    try
      ...
      try
        ...
        begin
          patientChartId := query.FieldByName('sz차트번호').AsString;
          patientName := query.FieldByName('sz이름').AsString;
          sexDstn := '0'; //GetGenderData( trim( query.FieldByName('b성별').AsString ) );
          birthday := ExtractInt( query.FieldByName('sz생년월일').AsString );
          addr := query.FieldByName('sz주소').AsString;
          addrDetail := ''; // query.FieldByName('Addr2').AsString;
          zipNo := query.FieldByName('sz우편번호').AsString; //StringReplace(query.FieldByName('ZipCode').AsString,'-','',[rfReplaceAll]);
          tel1 := query.FieldByName('sz휴대폰번호').AsString;
          tel2 := '';
          tel := tel1;
          cellPhone := ExtractInt(tel);
          regNum := ExtractInt(query.FieldByName('sz주민번호').AsString);
...
procedure TDentwebDBDM.Start;
  ...
  begin
    try
      ...
      try
        ADBConnectionString := dentweb.ConnectionString;
        Result := True;
      finally
        FreeAndNil( dentweb );
      end;
      ...
      begin
        Result := False;
        AddExceptionLog('TDentwebDBDM.Start.LoadDBConnectionString', e);
      end;
    end;
  end;
...
begin
  ...
  AddLog( doRunLog, 'TDentwebDBDM.testConnectDB');
  if testConnectDB(DBConnStr) then
  begin
    AddLog( doRunLog, 'TDentwebDBDM.Start DB Test OK!' );
  end
  else
  begin
    AddLog( doRunLog, 'TDentwebDBDM.Start DB Test Error' );
    exit;
  end;
...
procedure TDentwebDBDM.Stop;
begin
  if not FActive then
    exit;

  AddLog( doRunLog, 'TDentwebDBDM.Stop');

  FActive := False;

end;

function TDentwebDBDM.testConnectDB(DBConnStr: string) : Boolean;
var
  dbconn : TADOConnection;
begin
  Result := False;
  ...
  try
    ...
    try
      ...
      try
        ...
        on e : exception do
        begin
          AddExceptionLog('TDentwebDBDM.testConnectDB: dbconn.Connected := True; Failed.', e);
          if Assigned( FADOConn ) then
          begin
            FreeAndNil( FADOConn );
          end;
      ...
      AddLog( doValueLog, 'TDentwebDBDM.ConnectDB, Result=' + BoolToStr(Result, true) + ', ' + DBConnStr );
    except
      on e : exception do
      begin
        AddExceptionLog('TDentwebDBDM.ConnectDB', e);
...
procedure TDentwebDBDM.DisconnectDB;
begin
  if Assigned(FADOConn) then
