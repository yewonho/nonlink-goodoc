      ...
      case eventrequest.EventID of
        ...
        EventID_환자조회  :
          begin
            ...
            else if GetRREnv.HookOCSType = THookOCSType.None then     //없음 환자조회X일때 730응답
                begin
                 eventresponse := ResponseUnknownRequest(eventrequest.EventID, eventrequest.JobID, Result_NoRelation_NotDefineEvent, Result_NoRelation_NotDefineEvent_Msg);
                end
            else
...
begin
  TThread.CreateAnonymousThread(
    ...
    begin
      FObserver.BeforeAction( OB_Event_DataRefresh );
      try
       // GetEvent400Data;      //접수취소, 완료시 400을 호출하고있어서 일단 주석처리함. 문제시 풀어야됨
      finally
...
begin
  ...
  try
    try
      ...
      // FObserver.AfterAction(OB_Event_DataRefresh_Notify); // 미확정 내역 알림 팝업 미노출 굿닥 요청
