...
uses Classes, System.SysUtils, IdSSLOpenSSL, IdTCPClient, IdGlobal, IdCoderMIME,
     IdHash, IdHashSHA, math, System.threading, DateUtils, System.SyncObjs, IdURI, Winapi.Windows;
Type
...
//      *  %xB-F are reserved for further control frames
  ...
  public
    ...
    procedure Connect(pURL, pUniqueId:String);overload;
    procedure Close;
...
procedure TIdSimpleWebSocketClient.Connect(pURL, pUniqueId: String);
var  URI      : TIdURI;
     lSecure  : Boolean;
begin
  uri := nil;
  try
    ...
    if not string.IsNullOrEmpty(pUniqueId) then
    begin
      self.URL := self.URL + '?uniqueId=' + TIdURI.ParamsEncode(pUniqueId);
      URI.Params := 'uniqueId=' + TIdURI.ParamsEncode(pUniqueId);
    end;
    ...
    if not URI.Params.IsEmpty then
      self.Socket.WriteLn(format('GET %s HTTP/1.1', [uri.path+uri.Document+'?'+URI.Params]))
    else
...
begin
  ...
  // because... pHeader.Values['Connection'].Trim.ToLower is 'upgrade, keep-alive'
  //if (pHeader.Values['Connection'].Trim.ToLower='upgrade') and (pHeader.Values['Upgrade'].Trim.ToLower='websocket') then
  if (string.CompareText(pHeader.Values['Connection'].Trim.ToLower.Substring(0,7), 'upgrade') = 0) and (pHeader.Values['Upgrade'].Trim.ToLower='websocket') then
  begin
