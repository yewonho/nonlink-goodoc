...
//  TRnRDataState = (rrsRequest, rrsDecide, rrsRequestVisite, rrsVisiteDecide, rrsFinish );  // 진행 상태 type,  요청/확정/내원ㅝㄴ요청/내원확정/완료
  ...
  public
    ...
    class function RnRDataStatus2DispStr2( ARnRType : TRnRType; A접수확정: Boolean; ARnRDataStatus : TRnRDataStatus; ADeviceType: TRnRInDevice ) : string;  overload;
...
class function TRRDataTypeConvert.RnRDataStatus2DispStr2(
  ARnRType : TRnRType; A접수확정: Boolean; ARnRDataStatus: TRnRDataStatus; ADeviceType: TRnRInDevice): string;
begin
  case ARnRDataStatus of
    rrs예약실패, // Status_예약실패 = 'S02';
    rrs접수실패 : Result := '취소'; // Status_접수실패 = 'C02';
