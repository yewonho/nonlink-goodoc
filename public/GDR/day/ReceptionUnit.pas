...
type
  ...
  public
    ...
    //Button_내원요청 : TSakpungImageButton2;
...
//  if Assigned(Button_내원요청) then
//  begin
//    Button_내원요청.Parent := nil;
//    FreeAndNil( Button_내원요청 );
//  end;
//
...
begin
  ...
  begin
    ...
    if Decide_check.Checked then // 접수 확정
    begin
      Include(statusfilter, rrs접수완료);
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        exit;
//      end
//      else if AObj = rowdata.Button_내원요청 then
//      begin
//        ARow := i;
//        ATag := rowdata.Button_내원요청.Tag;
//        Result := rowdata;
//        exit;
//      end
  ...
  case rowdata.Status4App of
    ...
    csa환자내원         : // 내원 요청/내원 확인 버튼만 출력 한다.
      begin
        ...
        begin
//          rowdata.Button_내원요청.Visible := False;
...
////          dispButton(r1, rowdata.Button_내원요청);
////          rowdata.Button_내원요청.Enabled := not (rowdata.Status in [rrs내원요청]);
//            dispButton(workrect, rowdata.Button_진료완료);
//
//          r1 := s.CellRect(FDrawGrid.Col_Index_Button2, ARow);
////          dispButton(r1, rowdata.Button_내원확인);
//
//        end;
      end;
...
//            rowdata.Button_내원요청 := TSakpungImageButton2.Create(listgrid);
//            rowdata.Button_내원요청.Visible := False;
//            rowdata.Button_내원요청.Parent := listgrid;
//            rowdata.Button_내원요청.Tag := Index_tag_내원요청;
//            rowdata.Button_내원요청.PngImageList := ImageResourceDM.ButtonImageList;
//            ImageResourceDM.SetButtonImage(rowdata.Button_내원요청, aibtButton1, BTN_Img_내원요청);
//            //rowdata.Button_내원요청.OnClick := ButtonClickEvent;
//            rowdata.Button_내원요청.OnClick := ButtonClickEventThread;
...
//            rowdata.Button_내원확인.OnClick := ButtonClickEventThread;
  ...
  Label1.Caption := Format('접수 (%d명)',[cnt]);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      dtime := '확정시각' + Const_Char_Arrow_UPDown;
      case FSortType of
        ...
        dtime := '확정시각' + Const_Char_Arrow_UP;
      end;
//      str := Grid_Head_상태 + ',"' + dname + '","' + droom + '",' + Grid_Head_생일_주번 + ','+ Grid_Head_내원목적+ ',"' + dtime + '",' + Grid_Head_관리;
...
//        ColWidths[FDrawGrid.Col_Index_Symptom] := -1;
     ...
     (* if GetRREnv.ShowOptionReservationTime then
      begin
        ...
        환경설정에서 예약시간 체크 해제시 환자 리스트 나오지않아 비고처리- 후 정상 노출 확인 *)
